...
export class BpmnParser {
     ...
     */
    public static async generateConstantsForBpmnFiles(filenames: string | string[]): Promise<string> {
        if (typeof filenames === "string") {
            filenames = [filenames];
        }
...
`;
     ...
     */
    public static async getTaskTypes(processes: object[] | object): Promise<string[]> {
        const processArray: object[] = Array.isArray(processes) ? processes : [processes];
     ...
     * @param processes - A parsed BPMN object, or an array of parsed BPMN objects.
     */
    public static async getMessageNames(processes: object[] | object): Promise<string[]> {
        const processArray: object[] = Array.isArray(processes) ? processes : [processes];
                    ...
                    if (obj.hasOwnProperty(k)) {
                        if (k === "bpmn:serviceTask") {
                            ...
                            taskTypes = taskTypes.concat(
                                tasks.map((t) => t["bpmn:extensionElements"]["zeebe:taskDefinition"].attr["@_type"]));
                        } else {
