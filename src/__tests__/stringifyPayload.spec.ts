...
test('stringifyPayload returns a new object', () => {
	expect(stringifyPayload(jobObject)).not.toEqual(jobObject)
})

test('stringifyPayload stringifies the payload key of a job object', () => {
...
test('parsePayload returns a new object', () => {
	expect(parsePayload(jobDictionary)).not.toEqual(jobDictionary)
})

test('parsePayload parses the payload key of a job object to JSON', () => {
	expect(typeof parsePayload(jobDictionary).payload).toBe('object')
})

test('parsePayload correctly parses the payload string', () => {
	const parsed = parsePayload(jobDictionary)
	expect(parsed.payload.jobId).toEqual('7ead71d8-30c9-4eda-81e7-f2ada6d7d0da')
...
test('returns an object with all the keys of the original', () => {
	const parsed = parsePayload(jobDictionary)
